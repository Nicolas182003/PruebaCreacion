    ...
    <div class="space-y-6 animate-in fade-in duration-300">
          ...
          <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1.5 opacity-80">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Empresa Asignada (Real Time)</label>
              ...
              <div class="space-y-1.5" [class.opacity-40]="newUser.tipo === 'Admin'">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">División Asignada (Real Time)</label>
      ...
      <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden mt-8">
        <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <h3 class="text-xs font-black text-primary uppercase tracking-widest">Usuarios en {{ subName() }}</h3>
          ...
          <tbody class="divide-y divide-slate-100">
            @for (user of users(); track user.id) {
              <tr class="hover:bg-slate-50/30 transition-colors">
                ...
                <td class="px-8 py-4">
                  <span class="px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-tight">
                    {{ user.tipo }}
                  </span>
                </td>
...
export class UserManagementComponent implements OnInit, OnChanges {
  ...
  // Hacemos que los inputs también sean señales para que 'computed' reaccione a ellos
  ...
  // NOMBRES COMPUTADOS: Reaccionan a cambios en la jerarquía Y en los inputs
  ...
  subName = computed(() => {
    ...
    // Buscar específicamente en el padre
